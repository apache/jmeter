...
public class Start extends AbstractAction {
    ...
    public void doAction(ActionEvent e) {
        if (e.getActionCommand().equals(ActionNames.ACTION_START)) {
        ...
        } else if (e.getActionCommand().equals(ActionNames.RUN_TG) || e.getActionCommand().equals(ActionNames.RUN_TG_NO_TIMERS)) {
            popupShouldSave(e);
            ...
            if(nodes.length > 0) {
                ...
                LOGGER.warn("No thread group selected the test will not be started");
            }
        } 
    }
     ...
     */
    private void startEngine(boolean ignoreTimer, AbstractThreadGroup[] tg) {
        GuiPackage gui = GuiPackage.getInstance();
        HashTree testTree = gui.getTreeModel().getTestPlan();
        
        JMeter.convertSubTree(testTree);
        if(tg != null) {
            removeThreadGroup(testTree, tg);
        }
    ...
    /**
     * Remove thread groups from tree
     * @param testTree the tree
     * @param threadGroupsToKeep the thread group to keep
     */
    private void removeThreadGroup(HashTree testTree, AbstractThreadGroup[] threadGroupsToKeep) {
        ...
        for (Object o  : copyList) {
            ...
            if (o instanceof AbstractThreadGroup) {
                if (!isInThreadGroups(item, threadGroupsToKeep)) {
                    ...
                    removeThreadGroup(testTree.getTree(item), threadGroupsToKeep);
                }
            }
            else {
                removeThreadGroup(testTree.getTree(item), threadGroupsToKeep);
    ...
    private boolean isInThreadGroups(TestElement item, AbstractThreadGroup[] tg) {
        for (AbstractThreadGroup abstractThreadGroup : tg) {
