...
public class ReportGenerator {
    ...
    private static AggregateConsumer createEndDateConsumer() {
        AggregateConsumer endDateConsumer = new AggregateConsumer(
                new MaxAggregator(), sample -> ReportGeneratorConfiguration.jmeter_reportgenerator_msns_isMs ? (double) sample.getEndTime() : sample.getEndTime() / 1000000.0D);
    ...
    private static AggregateConsumer createBeginDateConsumer() {
        AggregateConsumer beginDateConsumer = new AggregateConsumer(
                new MinAggregator(), sample -> ReportGeneratorConfiguration.jmeter_reportgenerator_msns_isMs ? (double) sample.getStartTime() : sample.getStartTime() / 1000000.0D);
