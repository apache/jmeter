...
public abstract class HTTPSamplerBase extends AbstractSampler implements
    ...
    public URL getUrl() throws MalformedURLException {
        ...
        if (getPort() == UNSPECIFIED_PORT || getPort() == DEFAULT_HTTP_PORT) {
            ...
            return new URL(getProtocol(), getPropertyAsString(DOMAIN),
    ...
    protected HTTPSampleResult downloadPageResources(HTTPSampleResult res,
        ...
        try {
            if (res.getContentType().toLowerCase().indexOf("text/html") != -1) {
                urls = HTMLParser.getParser().getEmbeddedResourceURLs(
                        res.getResponseData(), res.getURL());
            }
        ...
        if (urls != null && urls.hasNext()) {
            ...
            while (urls.hasNext()) {
                ...
                try {
                    HTTPSampleResult binRes = sample((URL) binURL, GET, false,
                            frameDepth + 1);
    ...
    protected String encodeSpaces(String path) {
        ...
        path = Util.substitute((Perl5Matcher) localMatcher.get(), pattern,
                spaceSub, path, Util.SUBSTITUTE_ALL);
        return path;
    }
    ...
    protected HTTPSampleResult followRedirects(HTTPSampleResult res,
        ...
        for (redirect = 0; redirect < MAX_REDIRECTS; redirect++) {
            ...
            // this behaviour.
            try {
                lastRes = sample(new URL(lastRes.getURL(), location), GET,
                        true, frameDepth);
            } catch (MalformedURLException e) {
