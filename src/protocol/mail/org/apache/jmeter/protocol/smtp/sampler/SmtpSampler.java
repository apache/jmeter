...
public class SmtpSampler extends AbstractSampler {
    ...
    private boolean executeMessage(SampleResult result, SendMailCommand sendMailCmd, Message message) {
        ...
        try {
            ...
            log.warn("", mex);
            result.setResponseCode("500");
            result.setResponseMessage(getMessageFromMessagingException(mex));
        } catch (Exception ex) {
            log.warn("", ex);
            result.setResponseCode("500");
            result.setResponseMessage(getMessageFromException(ex));
        }
        return didSampleSucceed;
    }

    private String getMessageFromException(Exception ex) {
        if (failedToBuildTruststore(ex)) {
            return "Failed to build truststore - did not try to send mail!";
        } else {
            return "Other Exception: " + ex;
        }
    }

    private String getMessageFromMessagingException(MessagingException mex) {
        if (isTimeout(mex)) {
            return "MessagingException: Probably, SSL is not supported by the SMTP-Server!\n" + mex;
        } else if (isTLSFailure(mex)) {
            return "MessagingException: StartTLS not supported by server or initializing failed!\n" + mex;
        } else if (isCertificateNotFound(mex)) {
            return "MessagingException: Server certificate not trusted - perhaps restart JMeter!\n" + mex;
        } else {
            return "Other MessagingException: " + mex;
        }
    }
    ...
    private byte[] processSampler(Message message) throws IOException, MessagingException {
        ...
        // TODO - charset?
        return sb.toString().getBytes(); // TODO this should really be request data, but there is none
    }
    ...
    private boolean failedToBuildTruststore(Exception ex) {
        return ex.getMessage() != null
                // TODO: is this ever possible for an exception message to match?
                && ex.getMessage().contains("Failed to build truststore");
    }

    private boolean isCertificateNotFound(MessagingException mex) {
        return mex.getMessage().contains("send command to")
                && mex.getCause().getMessage().contains(
                "unable to find valid certification path to requested target");
    }

    private boolean isTLSFailure(MessagingException mex) {
        return mex.getMessage().contains("StartTLS failed");
    }

    private boolean isTimeout(MessagingException mex) {
        return mex.getMessage().contains("Could not connect to SMTP host")
                && mex.getMessage().contains("465")
                && mex.getCause().getMessage().contains("Connection timed out");
    }
