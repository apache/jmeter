...
public class Generate
{
    ...
    public String translate(String string)
    {
            ...
            && (-1 != (amp = string.indexOf('&', index))))
        {
            ...
            {
                System.out.println(
                    "unknown special character starting with "
                        + string.substring(amp, amp + 7));
                ret.append("&");
                index = amp + 1;
            }
        }
    ...
    public String pack(String string)
    {
        ...
        index = 0;
        while ((index < string.length())
            && (-1 != (spaces = indexOfWhitespace(string, index))))
        {
            ret.append(string.substring(index, spaces));
            ret.append(" ");
            while ((spaces < string.length())
                && Character.isWhitespace(string.charAt(spaces)))
                spaces++;
            index = spaces;
        }
