...
public class ExponentialTimer extends AbstractTestElement implements Cloneable, Timer, TestStateListener, TestBean, ThroughputProvider, DurationProvider {
    ...
    public long delay() {
        ...
        if (endTime > 0 && System.currentTimeMillis() + delay > endTime) {
            throw new JMeterStopThreadException("The thread is scheduled to stop in " +
                    ...
                    " JMeter (as of 2.9) does not support interrupting of sleeping threads, thus terminating the thread manually."
            );
        }
        return delay;
    }
    ...
    private EventProducer getEventProducer() {
        ...
        EventProducer eventProducer = groupEvents.get(tg);
        if (eventProducer == null) {
            // TODO: replace with Map.computeIfAbsent when Java 8+
            synchronized (groupEvents) {
                eventProducer = groupEvents.get(tg);
                if (eventProducer == null) {
                    Long seed = randomSeed == null || randomSeed == 0 ? null : randomSeed;
                    eventProducer = new ConstantPoissonProcessGenerator(new ThroughputProvider() {
                        @Override
                        public double getThroughput() {
                            return ExponentialTimer.this.getThroughput() / throughputPeriod;
                        }
                    }, batchSize, batchThreadDelay, this, exactLimit, allowedThroughputSurplus, seed, true);
                    groupEvents.put(tg, eventProducer);
                }
            }
        }
